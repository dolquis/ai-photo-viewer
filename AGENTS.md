...
- **状態運用**: 着手で `In Progress`、PR 作成で `In Review`、**PR マージで `Merged` へ（自動 `Done` にしない）**、`Done` は検証メモ記載後に明示遷移する（`docs/linear-conventions.md` §7.1.3）。人間ゲート課題は `In Review` / `Merged` に置かず `Todo` で待機、`type:tracking`（傘 Issue）は直接の実装対象にせず子完了まで `In Progress`（同 §3.1）。
