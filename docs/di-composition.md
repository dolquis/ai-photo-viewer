...
ここで正典との整合に注意が必要になる。
`AGENTS.md` §4 と `docs/architecture.md` §2 は「`App` → `UI` / `Core` を参照」と定めるが、具象を登録するには合成ルートが具象の在処（`Infrastructure` / `Database` / `Imaging` / `AI` / `Jobs` / `Search`）を参照する必要がある。
合成ルートが全モジュールの具象を知ること自体は DI の標準的な構造であり、依存方向の逆転を一点に閉じ込めるための意図的な例外である。
この例外をどう正典へ反映するかは §7 の未決事項とし、本書では合成ルートの責務（具象の登録と画面の解決）だけを先に確定する。
未決のあいだは、`App` から各機能層への参照追加は lead 判断を待つ。
...
能力問い合わせ口の具体形（契約 IF の置き場所と戻り値）は §7 の未決事項とし、本書では「無効化はまず UI ゲートで表現し、無効化サービスに結果を捏造させない」という方針までを確定する。
...
- **合成ルートの参照境界**（要 lead 判断）。§2 の例外を正典へどう反映するか。標準解は「合成ルートは全具象を参照してよい唯一の例外」と明記し、`architecture.md` §2 と `AGENTS.md` §4 を更新して `App` に各機能層への参照を許す案である。これを避けるなら、具象登録を集約する専用プロジェクト（`AiPhotoViewer.Composition` など）を新設し `App` → `UI` / `Core` / `Composition` とする案を採る。いずれも正典の依存規約に触れるため、本書では確定せず lead 判断を待つ。
...
合成ルートの参照境界と能力問い合わせ口の二点は、参照関係や契約 IF に影響し得るため lead 判断を要する。
