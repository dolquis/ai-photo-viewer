...
| HNSW 系（マネージド実装。**具体パッケージは TBD**） | 純マネージドの近似最近傍。`HNSW.Net` はリポジトリ名でパッケージ ID ではない（NuGet の ID は `HNSW`）。版によって対象フレームワークが異なるため、**`net8.0` 互換のパッケージ ID・バージョンを PoC-6 で特定**したうえで、メンテ状況・性能・メモリを実測して判断 |
